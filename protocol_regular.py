...
if __name__ == '__main__':
    ...
    rec_cntrl = spike_train_costs(time, tt, baseline_atp, spike_vals, ca_cntrl, special=True)
    rec_letm1 = spike_train_costs(time, tt, baseline_atp, spike_vals, ca_letm1, special=True)
    if MCU_KD:
        rec_mcukd = spike_train_costs(time, tt, baseline_atp, spike_vals, ca_mcukd)

    labels = ['Control', 'Letm1']
    t_ca = [7, 20]
    recs = [rec_cntrl, rec_letm1]
    if MCU_KD:
        labels.append('MCUKD')
        t_ca.append(7)
    ...
    for ii, rec in enumerate(recs):
        filename = str(baseline_atp) + '_regular_'+str(int(firing_freq))+'Hz_' + str(t_ca[ii]) + '_' + labels[ii] + '_600AP_2long.npz'
