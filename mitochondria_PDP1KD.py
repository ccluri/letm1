...
beta2 = lambda jjP=1: (1+jjP)*k2/k1*nt*pyrs # ca depn  PDH  
beta3 = k3/k1*accs*oaas
beta4 = lambda jjC=1: (1+jjC)*k4/k1*cits*nt  # ca depn isocitDH
beta5 = lambda jjK=1: (1+jjK)*k5/k1*kgs*nt*at  # ca depn  alpha-KGDH
beta6 = k6/k1*oaas
beta7 = k7/k1*pyrs*at
beta8 = k8/k1*oaas
...
dr2 = psim*a_m
Pis = 2.4e-3
batp = lambda jjV=1: (1+jjV)*katp/k1  # ca depn complex V
dc = frt*psim
datp = b_m*at
...
class Mito(object):
    def __init__(self, pyr=1, acc=1, cit=1, akg=1, oaa=1,
        ...
        self.set_calcium_depn_vars()
        self.reaction_rates(current_atp=baseline_atp, current_leak=kl)
        self.derivatives()
        self.ca_derivatives()
        
    def set_calcium_depn_vars(self):
        self.ca_mat = 0  # free ca excess in mito
        self.jjV = 0  # complex V acceleration factor
        self.jjP = 0  # PDH
        self.jjC = 0  # isoCDH
        self.jjK = 0  # AKGDH
    ...
    def reaction_rates(self, current_atp, current_leak):
        ...
        self.v1 = 1
        self.v2 = beta2(self.jjP)*self.nad*self.pyr  # ca depn
        self.v3 = beta3*self.acc*self.oaa
        self.v4 = beta4(self.jjC)*self.cit*self.nad  # ca depn
        self.v5 = beta5(self.jjK)*self.akg*(1-self.atp)*self.nad  # ca depn
        ...
        self.vatp = batp(self.jjV)*(2/(1+exp(datp*(self.atp-acrit)))-1) # ca depn
    ...
    def ca_derivatives(self):
        ...
        self.djjPdt = 0 # (jjinfp(self.ca_mat) - self.jjP) / 100000
    ...
    def update_vals(self, dt, atp_cost=0, leak_cost=0, ca_mat=0):
        ...
        self.psi = to_psi(self.dpsi)
        # ca depn updates
        self.jjV += self.djjVdt*taum
        self.jjP += self.djjPdt*taum
        self.jjC += self.djjCdt*taum
        self.jjK += self.djjKdt*taum
